...
static void alley_enter(struct scene *s)
{
	...
	d->windows[0] = (window_t){ 40,  70, 28, 24, ROOM_FISHBOWL, 1 };
	d->windows[1] = (window_t){ 90,  60, 28, 24, ROOM_KENNEL,   1 };
	d->windows[2] = (window_t){ 140, 70, 28, 24, ROOM_BIRDCAGE, 1 };
	d->windows[3] = (window_t){ 190, 60, 28, 24, ROOM_CHEESE,   1 };
	d->windows[4] = (window_t){ 240, 70, 28, 24, ROOM_LOVE,     1 };
	d->windows[5] = (window_t){ 280, 60, 28, 24, ROOM_MILK,     1 };
...
static void alley_update(struct scene *s, float dt)
{
	...
	for (int i = 0; i < WINDOW_COUNT; i++) {
		...
		if (d->cat.x + 12 > w->x && d->cat.x < w->x + w->w &&
		    d->cat.y + 4 > w->y && d->cat.y < w->y + w->h) {
	...
	for (int i = 0; i < 3; i++) {
		...
		if (d->other_cat_x[i] < 10 || d->other_cat_x[i] > 310)
...
static void alley_render(struct scene *s)
{
	...
	render_fill_rect(0, 60, SCREEN_W, 90, PAL_DGRAY);

	for (int i = 0; i < WINDOW_COUNT; i++) {
		window_t *w = &d->windows[i];
		uint8_t c = w->open && !game_room_is_done(w->room_type) ? PAL_YELLOW : PAL_DGRAY;
		render_fill_rect(w->x, w->y, w->w, w->h, PAL_BLACK);
		render_rect(w->x, w->y, w->w, w->h, c);

		if (!w->open || game_room_is_done(w->room_type)) {
	...
	for (int i = 0; i < 16; i++) {
		render_sprite(sprite_fence_top[rand() % 4], i * 20, 155, 8, 8);
	}
	render_fill_rect(0, 163, SCREEN_W, 5, PAL_BROWN);

	for (int i = 0; i < 5; i++) {
		render_sprite(sprite_trashcan_lid, d->trash_x[i], 110, 24, 12);
		render_sprite(sprite_trashcan_mid, d->trash_x[i] + 4, 122, 16, 8);
		render_sprite(sprite_trashcan_bot, d->trash_x[i] + 4, 130, 16, 11);
	}

	render_fill_rect(0, SCREEN_H - 16, SCREEN_W, 16, PAL_DGRAY);
	...
	for (int i = 0; i < 3; i++) {
		...
		render_sprite(cs, d->other_cat_x[i], 132, 24, 11);
	}

	player_render(&d->cat);

	if (d->entering_room) {
		render_text("ENTERING...", 120, 90);
	}
}
